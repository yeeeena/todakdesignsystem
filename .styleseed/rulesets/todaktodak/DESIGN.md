...
이 문서는 AI와 사람이 토닥토닥 화면을 만들 때 따르는 판단 기준이다. 위 frontmatter는 정확한 값이고, 본문은 언제 왜 쓰는지다. 값과 규칙은 최종본 화면 @prototype/todaktodak-working.html 을 실측해 뽑았고, 승인된 것만 담았다. 주제별 자세한 기준은 @docs/design-guide/INDEX.md 에서 찾고, 아직 정하지 않은 항목은 @docs/design-guide/FOLLOWUP.md 에 있다. 화면 상태와 분기, 고정 문구, 신별 콘텐츠는 이 문서가 다루지 않는다.

## Overview

토닥토닥은 토착신이 사용자의 고민을 읽고 작은 실천과 부적으로 남겨주는 감정 케어 서비스다. 인상은 크림 바탕과 차콜 잉크의 따뜻한 무채색이고, 일러스트가 화면의 절반을 차지한다. 색은 신별 몰입 순간에 아껴 쓰고, 그 밖의 위계는 잉크 농도와 굵기로 나눈다.

흐름은 선형이다. 한 화면의 주 행동은 하나이고, 여러 선택지를 대시보드처럼 펼치지 않는다. 하단 탭바를 두지 않는다.
...
위험과 경고 색은 안전 안내와 생성 실패 화면에만 쓴다. 이 서비스에서 빨강은 지금 도움이 필요하다는 신호다.
...
캔버스는 phone-w 폭 하나를 목표로 한다. 본문 좌우 여백은 spacing-24, 카드 안쪽 여백은 spacing-18, 카드 사이는 spacing-12 다. 간격은 spacing 척도 안의 값만 쓰고, 척도 밖 값을 새로 만들지 않는다.
...
고민 정리 3단계의 상단 일러스트는 신 목록에서 신을 골라 들어왔을 때만 그 신의 것을 쓰고, 그 밖에는 공통 일러스트를 쓴다(2026-08-05 결정). 입력 화면에서 신을 바꿔도 공통을 유지한다. 배경이 통째로 바뀌면 화면이 넘어간 것처럼 읽힌다.

## Elevation & Depth

떠 있는 요소는 적다. 카드와 시트는 그림자보다 면 색 차이와 얇은 선으로 구분한다. 그림자는 카드성 요소에 shadow-2, 토스트에 shadow-toast 만 쓴다. 하드 섀도는 시작 화면 버튼 한 곳의 예외이고, 다른 화면으로 번지지 않게 한다.
...
부품의 크기와 상태, 수치는 @docs/design-guide/컴포넌트/ 의 부품별 문서에 있다. 계약 파일(CONTRACT.md)은 아직 없어서 그 문서들이 자리를 대신한다. 여기서는 무엇을 언제 고르는지만 정한다.
...
주 행동은 화면당 하나이고 차콜 채움 버튼으로 둔다. 버튼이 세 개 필요하면 주 행동 하나를 전폭으로 위에, 보조 둘을 한 줄로 아래에 둔다(10, 11). 같은 위계의 버튼 세 개를 한 줄에 늘어놓지 않는다.
...
* [버튼](./컴포넌트/버튼.md) (계약 미작성)
* [하단 행동 영역](./컴포넌트/하단%20행동%20영역.md) (계약 미작성)
* [입력과 선택](./컴포넌트/입력과%20선택.md) (계약 미작성)
* [바텀시트](./컴포넌트/바텀시트.md) (계약 미작성)
* [캐릭터 링 로더](./컴포넌트/캐릭터%20링%20로더.md) (계약 미작성)
